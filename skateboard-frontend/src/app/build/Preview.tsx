...
export default function Preview({ wheelTextureURLs, deckTextureURLs }: Props) {
  ...
  // Use texture_url directly from Part type
  const wheelTexureURL = selectedWheel?.texture_url ?? DEFAULT_WHEEL_TEXTURE;
  const deckTexureURL = selectedDeck?.texture_url ?? DEFAULT_DECK_TEXTURE;
  // Use a default color for metals (trucks/bolts)
